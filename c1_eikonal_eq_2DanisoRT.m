...
%%%%%%%%%% Main QC parameters %%%%%%%%%%
% Some QC parameters that can be useful with real data... probably not
% necessary for the synthetic tests
maxstadist = 999; % maximum epicentral distance difference
minstadist = 0; % minimum epicentral distance difference
cohere_tol = 0; % 0.65 % minimum coherence allowed
min_stadist_wavelength = 0; %0.33; %0.5; % minimum number of wavelengths required between stations
max_stadist_wavelength = 999; % maximum number of wavelengths allowed between stations
ref_phv = 4*ones(size(parameters.periods)); % reference velocity for calculating wavelength
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Not needed for checkerboard tests
...
% aziweight = 1; % norm damping
aziweight = 999; % damp to zero for checkerboard test % norm damping
smweight0_azi = 0; % second derivative smoothing
flweight0_azi = 0; % first derivative smoothing
...
errorbar(periods,aniso_azi,aniso_azi_std*2,'-or'); hold on;
